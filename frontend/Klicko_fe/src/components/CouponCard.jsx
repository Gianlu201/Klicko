...
const CouponCard = ({ coupon }) => {
  return (
    <div className='relative bg-white border border-gray-400/40 rounded-xl overflow-hidden'>
      ...
      {coupon.isActive &&
        (coupon.percentualSaleAmount > 0 ? (
          <span className='absolute top-0 right-0 z-10 w-full transform rotate-45 translate-x-18 -translate-y-16 bg-primary text-white text-sm font-extrabold text-end py-1.5 pe-18 shadow-md'>
            -{coupon.percentualSaleAmount}%
          </span>
        ) : (
          <span className='absolute top-0 right-0 z-10 w-full transform rotate-45 translate-x-18 -translate-y-16 bg-primary text-white text-sm font-extrabold text-end py-1.5 pe-14 shadow-md'>
